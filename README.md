...
pre-setting AWS-style environment variables, copy
`backend.scaleway.tfbackend.example` to `backend.scaleway.tfbackend`, fill in
your Scaleway access/secret keys, and run:
...
multiple page bundles. Each page can specify a `repo`, default branch,
language, custom doc path, or manifest overrides; otherwise the CLI infers
sensible defaults (for example Rust repos assume `docs/users-guide.md` and
`Cargo.toml`).
...
regenerates `public/docs.html`, a docs hub styled to match `public/index.html`.
Descriptions on that landing page are fetched from project manifests (Cargo,
`pyproject.toml`, or `package.json`) unless overridden in the config file, so
adding new documentation bundles typically only requires declaring the repo
name and implementation language.
