...
const { createConfigKeys, nextTabIndex } = require(
...
describe("createConfigKeys: contract", () => {
    ...
    test("a key missing its label leaves the markup untouched", () => {
        ...
        expect(
            ui.notes().every(
                (n) => n.parentNode.getAttribute("data-config-keys-key") !== null
            )
        ).toBe(true);
    });
...
describe("createConfigKeys: wide mode", () => {
    test("the labels stay inert text and every panel is shown", () => {
        ...
        expect(ui.panels().map((p) => p.getAttribute("role"))).toEqual([
            "group",
            "group",
            "group",
        ]);
    ...
    test("no panel claims a description it no longer sits beside", () => {
        const ui = mount(true);
        expect(
            ui.panels().every((p) => p.getAttribute("aria-describedby") === null)
        ).toBe(true);
    });
});
