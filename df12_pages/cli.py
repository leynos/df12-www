...
app = App(name="pages", config=cyclopts.config.Env("INPUT_", command=False))
...
def generate(
    *,
    page: typ.Annotated[str | None, Parameter(help="Page identifier", env_var="INPUT_PAGE")] = None,
    config: typ.Annotated[Path, Parameter(help="Path to site config", env_var="INPUT_CONFIG")] = DEFAULT_CONFIG,
    source_url: typ.Annotated[str | None, Parameter(help="Override the page source URL", env_var="INPUT_SOURCE_URL")] = None,
    output_dir: typ.Annotated[Path | None, Parameter(help="Override the output folder", env_var="INPUT_OUTPUT_DIR")] = None,
) -> None:
    site_config = load_site_config(config)
    page_config = site_config.get_page(page)
    generator = PageContentGenerator(page_config, source_url=source_url, output_dir=output_dir)
