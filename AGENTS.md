...
The site is generated, not hand-written. Content lives in YAML and Markdown,
...
renders Jinja2 templates against structured data. Nothing in the published
site is authored directly as HTML.

### Where content comes from

- **`config/pages.yaml`** is the single source of layout and copy data. It
  defines the main site's homepage, about page, and documentation bundles, and
  a `sites:` mapping that configures each sub-site (`mxd`, `netsuke`,
  `weaver`). A sub-site entry declares its `output_dir`, `templates_dir`,
...
That token is then available as `text-night-muted` and carries its meaning
with it. Comment *why* the token exists, especially where it encodes a
constraint such as a contrast floor.

Repeated multi-class patterns should become a component class in the
sub-site's stylesheet rather than being copied between templates. Prefer a
daisyUI component where one fits.
...
- Tailwind compiles utilities into `@layer utilities`. Per CSS Cascade
  ...
  specificity, so an unlayered reset in a hand-crafted stylesheet will
  silently override Tailwind utilities. Wrap such rules in `@layer base`.
...
Rebuild and inspect the rendered result as well. A template change that
passes every gate can still produce a broken page, because the gates do not
render the site.
