...
semantically. The handwritten `himotoshi.css` rules move into the components
...
4. **Tailwind utilities in markup must beat semantic classes.** The
   handwritten rules go into `@layer components`; nothing is left unlayered.
...
- [x] (2026-09-03 16:10Z) Milestone 1: `home_page.jinja` extends
      ...
      **Addendum (2026-09-06):** #66 has since moved the shared chrome into
      `_layout.jinja`, which `home_page.jinja` now extends directly. The
      milestone's purpose — one copy of the chrome — holds; the file that
      owns it changed.
...
named tokens, and the templates refer to it semantically. The handwritten
...
`capture --site netsuke --width 360 --height 800`, and Hypothesis properties
for the transition, radius, incidental-text, transform and sibling-gap folds.
Two normalizer edges came out of it. A parent's `gap` was removed before the
fold was known to succeed, so a gap whose children carried `auto` margins
vanished from the comparison; it now stays unless a child absorbed it. And
every animation, colour-only ones included, was suppressing the boxes of its
...
`templates/netsuke/_layout.jinja` (the shared chrome every page extends,
...
1,560-line handwritten file copied verbatim into `public/`. Its last 150 lines
